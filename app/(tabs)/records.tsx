...
import { copyIntoMedikitDirAsync, deleteFileIfExistsAsync } from "@/utils/filesystem";
...
export default function RecordsScreen() {
  ...
  useEffect(() => {
    ImagePicker.requestMediaLibraryPermissionsAsync().then(({ status }) => {
      if (status !== "granted") {
        Alert.alert("Permission needed", "Please allow gallery access to attach prescriptions.");
      }
    });
  }, []);
  ...
  return (
    <Box className="flex-1 bg-white dark:bg-black px-4 py-6 gap-4">
      <View className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 gap-2">
        <Text className="text-lg font-semibold text-black dark:text-white">New record</Text>
        <TextInput
        ...
        <View className="gap-2">
          <View className="flex-row items-center justify-between">
            <Text className="text-base font-medium text-black dark:text-white">Attachments</Text>
            <Button size="sm" variant="outline" onPress={handlePickAttachment}>
              <ButtonText>Add photo</ButtonText>
            </Button>
          </View>
          {attachments.length === 0 ? (
            <Text className="text-sm text-slate-500">No files selected yet.</Text>
          ) : (
            <FlatList
              horizontal
              data={attachments}
      ...
      <View className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 gap-2 flex-1">
        <Text className="text-lg font-semibold text-black dark:text-white">Saved histories</Text>
        ...
        <FlatList
          ...
          keyExtractor={(item) => item.id}
          style={{ marginTop: 8 }}
          ItemSeparatorComponent={() => <View className="h-px bg-slate-200 dark:bg-slate-800 my-3" />}
          renderItem={({ item }) => (
            <View className="gap-1">
              ...
              {item.doctor ? <Text className="text-sm text-slate-500">{item.doctor}</Text> : null}
              {item.notes ? <Text className="text-sm text-slate-600">{item.notes}</Text> : null}
              <View className="flex-row justify-between items-center mt-2">
                <Text className="text-sm text-slate-500">{item.attachments.length} attachments</Text>
                <Button
                  size="sm"
                  variant="outline"
                  onPress={() =>
